...
X, y = make_circles(n_samples=n_samples, noise=0.03, random_state=81)
...
class BinaryCircleClasserV0(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.layer1 = nn.Linear(in_features=2, out_features=8, device=device)
        self.layer2 = nn.Linear(in_features=8, out_features=1, device=device)
        self.elu = nn.ELU()

    def forward(self, x):
        return self.layer2(self.elu(self.layer1(x)))
...
for epoch in range(epochs):
    ...
    if epoch % 10 == 0:
        ...
        print(f"Epoch: {epoch} | Accuracy: {accuracy:.2f}% | Loss: {loss.data} | Test Loss: {test_loss.data}")
